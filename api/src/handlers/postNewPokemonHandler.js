...
module.exports = async function postNewPokemonHandler(req,res){
    const { name, image, hp, attack, deffense, speed, height, weight, types} = req.body;
    if(!name, !image, !hp, !attack, !deffense, !types){
        return res.status(404).json({message: "Faltan los datos necesarios para completar el pokemon"})
    }
    ...
    try{
        ...
        if(!areThereTypes.length) {await axios.get('http://localhost:3001/types')}

        const existingTypes = await PokemonType.findAll({where: 
                {
